...
import { SpinnerSVG } from "../admin/Navbar";
    ...
    <header className="flex grow items-center border py-2 px-4 rounded-2xl border-gray-300 shadow-lg justify-between">
        ...
        <nav className="text-blue-600">
          <ul className="flex gap-4 font-semibold">
            <li className={`relative group flex py-2 px-4 rounded-2xl items-center gap-2 ${pathname === '/dashboard' ? 'text-white bg-blue-600' : 'bg-white text-blue-600'} hover:scale-105 active:scale-95 hover:text-white hover:bg-blue-600 transition-all ease-in-out duration-300`}>
              <Link
                onClick={() => {
                  setLoading('loading1');
                }}
                ...
                >{home}Dashboard{loading === 'loading1' && SpinnerSVG}</Link>
            </li>
            <li className={`relative group flex py-2 px-4 rounded-2xl items-center gap-2 ${pathname === '/sewa-kamar' ? 'text-white bg-blue-600' : 'bg-white text-blue-600'} hover:scale-105 active:scale-95 hover:text-white hover:bg-blue-600 transition-all ease-in-out duration-300`}>
              <Link 
                onClick={() => {
                  setLoading('loading2');
                }}
                href={`/sewa-kamar`} 
                className="flex gap-2 items-center"
                >{kamar}Sewa kamar{loading === 'loading2' && SpinnerSVG}</Link>
            </li>
            <li className={`relative group flex py-2 px-4 rounded-2xl items-center gap-2 ${pathname === '/tagihan' ? 'text-white bg-blue-600' : 'bg-white text-blue-600'} hover:scale-105 active:scale-95 hover:text-white hover:bg-blue-600 transition-all ease-in-out duration-300`}>
              <Link 
                ...
                href={`/tagihan`} 
                className="flex gap-2 items-center"
                >{tagihan}Tagihan{loading === 'loading3' && SpinnerSVG}</Link>
            </li>
          </ul>
        </nav>
      </header>
