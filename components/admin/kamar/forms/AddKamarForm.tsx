...
const InputKamarForm = ({
  ...
  const [data, action, pending] = useActionState(addKamar, null)


  return (
    <>
      {!data &&
        <div key={1} ref={overlayRef} className="backdrop-blur-xs absolute inset-0 flex justify-center items-center" onClick={e => e.target === overlayRef.current && setDisplayInputKamarForm(!displayInputKamarForm)}>
